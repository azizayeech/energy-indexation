...
return [
    ...
    'info' => [
        ...
        API REST para la consulta y cálculo de precios indexados de energía.

        La API permite calcular el precio indexado de un período utilizando:

        - consumos horarios;
        - precios horarios OMIE;
        - una fórmula personalizada que debe contener `[OMIE_MD]`.

        ### Fórmula de cálculo

        Para cada día y cada hora se evalúa la fórmula utilizando el precio OMIE correspondiente y se multiplica por el consumo horario.

        El precio indexado final se obtiene dividiendo el importe total entre el consumo total.

        ### Seguridad

        Las fórmulas se procesan mediante un analizador matemático restringido y no mediante `eval()`, evitando la ejecución de código arbitrario.

        El endpoint de cálculo dispone además de validación de entrada y limitación de solicitudes.
        MARKDOWN,
    ],
